...
const StatisticalData: React.FC<StatisticalDataProps> = ({ statistics }) => {
  return (
    <div className="space-x-4 space-y-0.5">
      ...
      <p>ยอดสั่งซื้อเข็มที่ระลึกทั้งหมด (ยืนยันการชำระเงินแล้ว): {statistics.totalCardOrdersApproved}</p>
      ...
      <p>ผู้บริจาคที่สั่งซื้อทั้งเสื้อที่ระลึกและเข็มที่ระลึก: {statistics.donatorsWithBothOrders}</p>
      <p>ยอดสั่งซื้อเข็มที่ระลึกทั้งหมด: {statistics.totalCardOrders}</p>
      <p>ยอดสั่งซื้อเข็มพร้อมกล่องทั้งหมด: {statistics.totalCardwithboxOrders}</p>
